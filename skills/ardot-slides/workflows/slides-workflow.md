...
### 1.0 Establish the Design File Context

**Follow `ardot-design-core` SKILL.md → Step 0** for the full rule — including the empty-canvas
note (root `0:1`, skip `fetch_editor_state`). Do not re-derive the file-context logic here.

> 🔧 **PORT NOTE.** 原版假设宿主注入 `<ardot_file_directive>`，且 `create_design` / `open_design`
> 一定存在。**默认通路 A（Ardot 客户端）两样都没有。** 出这里：
> - 目标文件 = **用户粘贴的链接**，且必须 `fetch_file_info` 核对。
> - 没有链接 → 先问。
> - 通路 A 一次只能操作客户端里打开的那一个文件。

**Slides-specific deviation — `fetch_file_info` 时机：** 原版"延后到 1.4"是为了覆盖
WorkBuddy 建/开文件的异步窗口。通路 A 没有建/开工具，**在 1.0 就调一次确认目标**
（`fetch_editor_state` 也行），不必等到 1.4。

### 1.1 Load Reference Knowledge (read these files if not already loaded)

> These two files live in the **ardot-design-core** skill (a sibling skill of this one).
> ⚠️ 路径是**相对本文件所在目录**算的。本文件在 `workflows/` 子目录里，比技能根深一层，
> 所以要上**两**级 `../../ardot-design-core/`；从技能根的 `SKILL.md` 出发才是一级 `../`。
...
**Skip this step for a brand-new empty file** — empty canvas, root is `0:1`, no variables, no components yet.  
🔧 *port note:* 通路 A 上没有 `create_design`，新建的空文件只能由用户在客户端/网页端先建好再打开。 There is nothing to fetch.
