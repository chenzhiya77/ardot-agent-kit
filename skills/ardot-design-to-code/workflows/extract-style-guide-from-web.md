...
## Phase 0: Establish the Design File Context

**Follow `ardot-design-core` SKILL.md → Step 0** for the full rule.
...
**Design-to-code deviation — `fetch_file_info` 时机：** 原版"延后到 Phase 2"是为了覆盖
WorkBuddy 建/开文件的异步加载窗口。通路 A 没有建/开工具，所以**在 Phase 0 就调一次确认
目标文件与权限**即可，不必延后。
