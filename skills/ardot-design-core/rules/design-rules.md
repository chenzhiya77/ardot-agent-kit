...
- **Override caveat**: as soon as you explicitly set `layout` (to `horizontal` or `vertical`) without also setting `width` / `height`, sizing falls back to `FIXED`, not `hug_contents`. So if you want dynamic sizing alongside an explicit `layout`, set `width` / `height` (to `fill_container` or `hug_contents`) explicitly in the same call.
...
- For single-line text, set `textAutoResize: "WIDTH_AND_HEIGHT"` to resizing, make sure text would not overflow.
...
Bind reusable design tokens to node properties with the **`$:<SetName>:<VariableName>`** syntax. The `SetName` is the variable set (collection) name and `VariableName` is the variable name within that set. Use `fetch_editor_state` to discover available variables (`usableVariables`), or `apply_variables` to create new ones. Variable types: `FLOAT`, `COLOR`, `BOOLEAN`, `STRING`.
...
| Text overflows | `textAutoResize: "WIDTH_AND_HEIGHT"` | Set `textAutoResize: "HEIGHT"` + `width: "fill_container"` (or fixed width)|
